...
//                print_r($attributed->toArray());
    ...
    /**
     * Upload action called when file upload to server
     */
    public function uploadAction() {
        ...
        try {
            if (!empty($_FILES)) {
                ...
                $extension = end($temp);
                $notllowedExts = array('php', 'pl', 'sh');

                if(in_array($extension, $notllowedExts)) {
                    header("HTTP/1.0 500 Server Error");
                    die();
                }

                $allowedExts = array('jad','cod','sis','sisx','apk','cab','mp3','jar','ipk','wgz','prc','jpg','jpeg','gif','png','bar','txt','mp4', 'pdf');
                ...
                if ($this->_request->isPost()) {
                    ...
                    $buildData = array('id' => (!empty($buildId) && $buildId != 'undefined' ) ? $buildId : null, 'product_id' => $productId, 'name' => $buildName);
                    $newBuildId = $productBuild->save($buildData);
                ...
                $logger = new Nexva_Util_Log_Mongo('file_upload');
                $data   = array(
                    'data'  => $buildData,
                    'msg'   => (string)$ex,
                    'trace'  => print_r($ex->getTrace(), true)
...
//        print_r($categories);
     ...
     public function uploadFilesAction(){
        ...
        if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0 && $this->_request->isPost())
        {
            ...
            $extension = end($temp);
            $notAllowedExts = array('php', 'pl', 'sh', 'zip', 'rar');
            ...
            $allowedFileExtensions = array('jad','cod','sis','sisx','apk','cab','mp3','jar','ipk','wgz','prc','jpg','jpeg','gif','png','bar','txt','mp4', 'pdf');
