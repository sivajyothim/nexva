...
$(document).ready(function(){
	...
	$("#stats_view_by_date, #stats_view_app_comparison,#app_wall").bind("plothover", function (event, pos, item) {
        if (item) { 
...
function appWallChart() {   
   
    if (typeof __DATA_APP_WALL == 'undefined' && typeof __APP_WALL_CHAPS == 'undefined') {
        return false;
    }    
    ...
    for(var i=0; i < __APP_WALL_CHAPS.length;i++){        
        var chap_array=[]; 
        for (var q=0; q < __DATA_APP_WALL[__APP_WALL_CHAPS[i]['meta_value']].length; q++) { 
            chap_array.push([ __DATA_APP_WALL[__APP_WALL_CHAPS[i]['meta_value']][q]['date'],__DATA_APP_WALL[__APP_WALL_CHAPS[i]['meta_value']][q]['count'] ]);
          
        }
        plots.push({data: chap_array, label: __APP_WALL_CHAPS[i]['meta_value']});
        setTimeout(1);
    }  
    
    var plot = $.plot($("#app_wall"), plots, options);
    showOverview(plot, $("#app_wall"), plots, $("#overview_app_wall"), options);

    __DATA_APP_WALL = __APP_WALL_CHAPS = chap_array  = null;
}