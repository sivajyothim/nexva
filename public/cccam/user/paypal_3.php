...
$currency = $result_query['paypal_cur_3'];
...
switch ($_GET['action']) {
   ...
   case 'process':      // Process and order...
      ...
      $p->add_field('item_name', 'Quota Socio : '.$_SESSION['username'].' , 3 mesi ');
   ...
   case 'success':      // Order was successful...
	  ...
	  $new_date = strtotime(date("d-m-Y", strtotime($old_date)) . " +3 month");
	  $value = date("d-m-Y",$new_date);
	  
	  // Update the database with new date
	  
	  $new_quote = "75"; // Change if need
	  $today = date("d-m-Y");
