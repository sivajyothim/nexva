...
//This is a custom validation for validate buildname in the CP panel
$.fn.ketchup.validation('buildname', function(element, value) {
    //if(/^([a-zA-Z])[a-zA-Z_-]*[\w_-]*[\S]$|^([a-zA-Z])[0-9_-]*[\S]$|^[a-zA-Z]*[\S]$/.test(value)) return true;
    if(/^[a-zA-Z0-9\s_-]+$/.test(value)) return true;
    else return false;
});

$.fn.ketchup.validation('briefdescription', function(element, value, minlength, maxlength) {
    //if(/^([a-zA-Z])[a-zA-Z_-]*[\w_-]*[\S]$|^([a-zA-Z])[0-9_-]*[\S]$|^[a-zA-Z]*[\S]$/.test(value)) return true;
    if(value.length >= minlength && value.length <= maxlength) return true;
    else return false;
});
