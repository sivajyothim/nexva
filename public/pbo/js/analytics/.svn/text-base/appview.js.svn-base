...
$(document).ready(function(){
	...
	function initViewByDateChart() {
		...
		plots.push({data: pointsView, label: 'Views'});
	...
	$("#stats_view_by_date, #stats_view_app_comparison,#stats_view_by_date_and_source,#stats_download_by_date_and_source").bind("plothover", function (event, pos, item) {
        ...
        if (item) { 
            if (previousPoint != item.datapoint) {
                ...
                var x = item.datapoint[0],
                    y = item.datapoint[1];
                showTooltip(item, y + ' ' +item.series.label+ ' - ' + getFormattedMonth(x));
            }
        }
        else {
            $("#tooltip").remove();
    ...
    //views by source chart
    function initViewSourceChart()
    {
        if (typeof __DATA_WEB_VIEWS == 'undefined' && typeof __DATA_MOBILEWEB_VIEWS == 'undefined' && typeof  __DATA_API_VIEWS == 'undefined') {return false;}
        ...
        for (date in __DATA_API_VIEWS) {
            pointsApp.push([date, __DATA_API_VIEWS[date]]);
        }
        plots.push({data: pointsApp, label: 'Android App'});

        for (date in __DATA_MOBILEWEB_VIEWS) {
            pointsMobileWeb.push([date, __DATA_MOBILEWEB_VIEWS[date]]);
        }
        plots.push({data: pointsMobileWeb, label: 'Mobile Web'});

        for (date in __DATA_WEB_VIEWS) {
            pointsWeb.push([date, __DATA_WEB_VIEWS[date]]);
        }
        plots.push({data: pointsWeb, label: 'Web'});

        $.plot($("#stats_view_by_date_and_source"), plots, options);

        __DATA_WEB_VIEWS = __DATA_MOBILEWEB_VIEWS = __DATA_API_VIEWS = pointsApp = pointsWeb = pointsMobileWeb = null;
    }

    //downloads by source chart
    function initDownloadSourceChart()
    {
        if (typeof __DATA_API_DOWNLOADS == 'undefined' && typeof  __DATA_MOBILEWEB_DOWNLOADS == 'undefined') {return false;}
        ...
        for (date in __DATA_API_DOWNLOADS) {
            pointsApp.push([date, __DATA_API_DOWNLOADS[date]]);
        }
        plots.push({data: pointsApp, label: 'Android App'});

        for (date in __DATA_MOBILEWEB_DOWNLOADS) {
            pointsMobileWeb.push([date, __DATA_MOBILEWEB_DOWNLOADS[date]]);
        }
        plots.push({data: pointsMobileWeb, label: 'Mobile Web'});

        $.plot($("#stats_download_by_date_and_source"), plots, options);

        __DATA_API_DOWNLOADS  = __DATA_MOBILEWEB_DOWNLOADS = pointsApp  = pointsMobileWeb = null;
    }
